...
/**
 * @package Codeigniter
 * @subpackage M_trasnformasi
 * @category Model
 * @author Author Name <author@developer.org>
 */

class M_trasnformasi extends CI_Model
{
	public function trasnformasi()
	{
        ...
        foreach ($nim as $key => $value) {
          ...
          $l =$this->db->select('layanan.id as idl')
          ->from('penilaian')
          ->join('layanan','layanan.id=penilaian.id_layanan')
          ->get()->result_array();
        foreach ($l as $key => $la) {
            $j = $this->db->select('*')->from('vote')->join('penilaian','penilaian.id=vote.id_penilaian')
            ->join('layanan','layanan.id=penilaian.id_layanan')->join('kategori','kategori.id=vote.id_kategori')
       ...
       $data_tes= $this->db->get('tranformasi')->result_array();
        foreach ($data_tes as $tes) {
         ...
         $pindah=$this->db->order_by('tranformasi.id', 'DESC')->get('tranformasi', 17)->result_array();
        foreach ($pindah as $p) {
        	$this->db->insert('data_uji',['label'=>$p['label'],'nim'=>$p['nim']]);
        }
        $kos=$this->db->order_by('tranformasi.id', 'DESC')->get('tranformasi', 17)->result_array();
        foreach ($kos as $kk) {
        	$this->db->update('tranformasi',['label'=>''],['nim'=>$kk['nim']]);
        }
       ...
       foreach ($label as $labell) {
        // $labelt[]=['nilai'=>$this->db->where('label',$labell['label'])->count_all_results('tranformasi'),'kategori'=>'total data'];
          if ($labell['label']!=null) {
            $tipe='Data Training';
            $x=[$labell['label']];
            $xx=$this->db->where('label',$labell['label'])->count_all_results('tranformasi');
          }else{
            $tipe='Data Uji';
            $xx=12;
          }
            $this->db->insert('hitung',['nilai'=>12,'kategori'=>$tipe,'from'=>$labell['label']]);
       }
       foreach ($l1 as $ll1) {
        if ($ll1['label']!=null) {
...
/* End of file M_trasnformasi.php */
/* Location : ./application/models/M_trasnformasi.php */