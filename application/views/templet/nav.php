<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <title>Naivfe Bayes</title>
  <!-- Tell the browser to be responsive to screen width -->
  <meta name="viewport" content="width=device-width, initial-scale=1">
  
  
  
  <!-- prettify.min.js
  prettify.min.css -->
  
<script src="<?=base_url('')?>assets/AdminLTE-master/plugins/jquery/jquery.min.js"></script>
  <link href="<?=base_url('assets/')?>ICON.png" rel="shortcut icon" type="image/ico" />

<script src="<?=base_url('')?>assets/AdminLTE-master/plugins/raphael/raphael.min.js"></script>
<script src="<?=base_url('')?>assets/morris/morris.min.js"></script>
<link rel="stylesheet" src="<?=base_url('')?>assets/morris/morris.css"></link>
 <!-- SweetAlert2 -->
 <link rel="stylesheet" href="<?=base_url('')?>assets/AdminLTE-master/plugins/sweetalert2-theme-bootstrap-4/bootstrap-4.min.css">  
<!-- Font Awesome -->
  <link rel="stylesheet" href="<?= base_url('assets/AdminLTE-master')?>/plugins/fontawesome-free/css/all.min.css">
  <!-- Ionicons -->
  <link rel="stylesheet" href="https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css">
  <!-- overlayScrollbars -->
  <link rel="stylesheet" href="<?= base_url('assets/AdminLTE-master')?>/dist/css/adminlte.min.css">
  <!-- Google Font: Source Sans Pro -->
  <link href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700" rel="stylesheet">
    <!--Data tabel -->
  <link rel="stylesheet" href="<?=base_url('')?>assets/plugin/plugins/DataTables/datatables-responsive/css/responsive.bootstrap4.min.css"><!-- Theme style -->
  <link rel="stylesheet" href="<?=base_url('')?>assets/plugin/plugins/DataTables/datatables.min.css">
</head>
<body class="hold-transition sidebar-mini">
<!-- Site wrapper -->
<div class="wrapper">
  <!-- Navbar -->
  <nav class=" main-header navbar navbar-expand navbar-dark navbar-primary">
    <!-- Left navbar links -->
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" data-widget="pushmenu" href="#" role="button"><i class="fas fa-bars"></i></a>
      </li>
      
    </ul>

    

    <!-- Right navbar links -->
    <ul class="navbar-nav ml-auto">
    
    </ul>
  </nav>
  <!-- /.navbar -->
